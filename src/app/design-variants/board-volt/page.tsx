...
function VoltBoardCard({ card }: { card: MockCard }) {
  return (
    <div
      className={`cursor-grab border border-neutral-800 border-l-2 p-3.5 transition-colors hover:border-neutral-600 ${priorityEdge[card.priority]}`}
    >
  ...
  return (
    <aside className="hidden w-60 shrink-0 flex-col border-r border-neutral-800 lg:flex">
      ...
      <div className="mt-2 flex-1 space-y-0.5 overflow-y-auto px-3">
        ...
        <button className="flex w-full items-center gap-3 px-3 py-2 text-sm text-neutral-600 transition-colors hover:text-neutral-300">
      ...
      <div className="border-t border-neutral-800 px-3 py-4">
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          <span className="flex size-6 items-center justify-center border border-neutral-700 font-mono text-[10px] text-neutral-300">
            KS
          </span>
...
export default function BoardVoltPage() {
  return (
    <div className="flex h-dvh bg-neutral-950 text-neutral-100">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-neutral-800 px-5 py-3">
          <label className="flex h-8 w-56 items-center gap-2 border border-neutral-800 px-2.5 focus-within:border-lime-300">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button className="flex h-8 items-center gap-1.5 border border-lime-300 px-3.5 text-sm font-semibold text-lime-300 transition-colors hover:bg-lime-300 hover:text-neutral-950">
        ...
        <div className="min-h-0 flex-1 overflow-auto">
          <div className="flex h-full min-w-max items-stretch gap-6 px-6 py-4">
            {boardColumns.map((column) => (
              <section key={column.name} className="flex w-72 flex-col">
                <div
                  className={`flex items-baseline justify-between border-t-2 px-0.5 pb-4 pt-3 ${
                ...
                <div className="space-y-2">
                  {column.cards.map((card) => (
                    <VoltBoardCard key={card.title} card={card} />
                  ))}
                </div>
                <button className="mt-2 flex w-full items-center gap-1.5 px-1 py-2 text-sm text-neutral-600 transition-colors hover:text-lime-300">
                  <IconPlus size={13} strokeWidth={1.5} />
                  Add card
                </button>
              </section>
            ...
            <section className="flex w-72 flex-col">
              <button className="flex items-center gap-1.5 border-t-2 border-neutral-800 px-0.5 pt-3 text-[11px] uppercase tracking-[0.14em] text-neutral-600 transition-colors hover:text-lime-300">

...
function CobaltBoardCard({ card }: { card: MockCard }) {
  return (
    <div
      className={`cursor-grab border border-l-2 border-neutral-800 bg-[#111318] p-3.5 transition-colors hover:border-neutral-600 ${priorityEdge[card.priority]}`}
    >
      ...
      <div className="mt-2.5 flex items-center gap-3 font-mono text-[10px] text-neutral-600">
        ...
        <span className="ml-auto flex items-center gap-3">
          ...
          {card.assignee ? (
            <span className="text-blue-400">{card.assignee.initials}</span>
          ) : null}
        </span>
      </div>
    </div>
...
export default function BoardCobaltPage() {
  return (
    <div className="flex h-dvh bg-neutral-950 text-neutral-100">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-neutral-800 px-5 py-3">
          <label className="flex h-8 w-56 items-center gap-2 border border-neutral-800 px-2.5 focus-within:border-blue-400/60">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button className="flex h-8 items-center gap-1.5 bg-blue-600 px-3.5 text-sm font-medium text-white transition-colors hover:bg-blue-500">
        ...
        <div className="min-h-0 flex-1 overflow-auto">
          <div className="flex h-full min-w-max items-stretch divide-x divide-neutral-800">
            {boardColumns.map((column) => (
              <section key={column.name} className="flex w-72 flex-col p-4">
                <div className="flex items-baseline justify-between px-0.5 pb-4">
                  <h2 className="font-mono text-[11px] text-neutral-500">
                    {column.name}
                  </h2>
                ...
                <div className="space-y-2">
                  {column.cards.map((card) => (
                    <CobaltBoardCard key={card.title} card={card} />
                  ))}
                </div>
                <button className="mt-2 flex w-full items-center gap-1.5 px-1 py-2 text-sm text-neutral-600 transition-colors hover:text-blue-400">
                  <IconPlus size={13} strokeWidth={1.5} />
                  Add card
                </button>
              </section>
            ...
            <section className="flex w-72 flex-col p-4">
              <button className="flex items-center gap-1.5 px-0.5 font-mono text-[11px] text-neutral-600 transition-colors hover:text-blue-400">

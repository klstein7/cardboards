...
function SignalBoardCard({ card }: { card: MockCard }) {
  return (
    <div
      className={`cursor-grab border-l-2 bg-neutral-900 p-3.5 transition-colors hover:bg-neutral-800 ${priorityEdge[card.priority]}`}
    >
    ...
    <aside className="hidden w-60 shrink-0 flex-col border-r border-neutral-800 lg:flex">
      ...
      <div className="border-t border-neutral-800 px-3 py-4">
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          <span className="flex size-6 items-center justify-center bg-lime-300 font-mono text-[10px] font-semibold text-neutral-950">
            KS
          </span>
  ...
  return (
    <div className="flex h-dvh bg-neutral-950 text-neutral-100">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-11 shrink-0 items-center gap-2 border-b-2 border-lime-300 px-5 text-sm">
          <Link
          ...
          <span className="text-neutral-700">/</span>
          <span className="flex items-center gap-2 font-semibold uppercase tracking-tight text-white">
            <span
              className="size-2"
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-3 border-b border-neutral-800 px-5 py-3">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button className="flex h-8 items-center gap-1.5 bg-lime-300 px-3.5 text-sm font-semibold text-neutral-950 transition-colors hover:bg-lime-200">
        ...
        <div className="min-h-0 flex-1 overflow-auto">
          <div className="flex h-full min-w-max items-stretch divide-x divide-neutral-800">
            {boardColumns.map((column) => (
              <section key={column.name} className="flex w-72 flex-col p-4">
                <div className="flex items-baseline justify-between px-0.5 pb-4">
                  <h2 className="text-[11px] font-semibold uppercase tracking-[0.14em] text-neutral-300">
                    {column.name}
                  </h2>
                ...
                <div className="space-y-2">
                  {column.cards.map((card) => (
                    <SignalBoardCard key={card.title} card={card} />
                  ))}
                </div>
            ...
            <section className="flex w-72 flex-col p-4">
              <button className="flex items-center gap-1.5 px-0.5 text-[11px] uppercase tracking-[0.14em] text-neutral-600 transition-colors hover:text-lime-300">
